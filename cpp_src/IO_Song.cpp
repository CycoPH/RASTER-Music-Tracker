	...
	if (dlg.DoModal() == IDOK)
	{
		...
		switch (formatChoiceIndexFromDialog)
		{
			...
			case FILE_EXPORT_FILTER_IDX_SAPR:
				...
				exportResult = Export(out, m_lastExportType, (char*)(LPCTSTR)fn);
				break;
...
bool CSong::Export(ofstream& ou, int iotype, char* filename)
{
	...
	if (maxadr < 0) return false;					// If the module could not be created, the export process is immediately aborted
	CString s;
